...
const Dashboard = () => {
  ...
  const handleCreateAsset = async (event: FormEvent) => {
    ...
    try {
      ...
      const ipfsUris = await uploadMedia([selectedFile]);
      const imageUrl = ipfsUris[0]; // Use the IPFS URI directly

      console.log("Uploaded to IPFS:", imageUrl);

      toast({
        title: "Minting Asset...",
        description: "Registering your asset on Story Protocol.",
      });
      ...
      const response = await fetch(`${API_URL}/assets`, {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ...
            metadataUri: imageUrl, // For now using image URI as metadata URI for simplicity, or we should upload full metadata
            creatorWallet: address,
            royalties: [] // Add royalties UI if needed
          },
        }),
      });
